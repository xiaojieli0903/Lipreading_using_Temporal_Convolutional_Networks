...
class MyDataset(Dataset):
    ...
    def _get_files_for_partition(self):
        ...
        search_str_npz = os.path.join(dir_fp, '*', self._data_partition,
                                      '*.npz')
        search_str_npy = os.path.join(dir_fp, '*', self._data_partition,
                                      '*.npy')
        search_str_mp4 = os.path.join(dir_fp, '*', self._data_partition,
                                      '*.mp4')
        ...
        self._data_files = [
            ...
            if f.split('/')[self.label_idx] in self._labels
        ]
    ...
    def _apply_variable_length_aug(self, filename, raw_data):
        ...
        info_txt = os.path.join(
            ...
            *filename.split('/')[self.label_idx:])  # swap base folder
        ...
        half_interval = int(utterance_duration / 2.0 *
                            self.fps)  # num frames of utterance / 2
        ...
        left_idx = random.randint(
            0, max(0, mid_idx - half_interval -
                   1))  # random.randint(a,b) chooses in [a,b]
        right_idx = random.randint(min(mid_idx + half_interval + 1, n_frames),
                                   n_frames)
...
def pad_packed_collate(batch):
    if len(batch[0]) == 2:
        use_boundary = False
        data_tuple, lengths, labels_tuple = zip(*[(a, a.shape[0], b) for (
    ...
    if use_boundary:
        boundaries_np = np.zeros(
            (len(boundaries_tuple), len(boundaries_tuple[0])))
        for idx in range(len(data_np)):
