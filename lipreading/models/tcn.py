...
class Chomp1d(nn.Module):
    ...
    def forward(self, x):
        ...
        if self.symm_chomp:
            return x[:, :,
                     self.chomp_size // 2:-self.chomp_size // 2].contiguous()
        else:
    ...
    def __init__(self,
        ...
        if dwpw:
            self.conv = nn.Sequential(
                ...
                if relu_type == 'prelu' else nn.ReLU(inplace=True),
                # -- pw
            ...
            self.non_lin = nn.PReLU(
                num_parameters=n_outputs) if relu_type == 'prelu' else Swish(
...
class MultibranchTemporalBlock(nn.Module):
    def __init__(self,
        ...
        for k_idx, k in enumerate(self.kernel_sizes):
            cbcr = ConvBatchChompRelu(n_inputs,
        ...
        for k_idx, k in enumerate(self.kernel_sizes):
            cbcr = ConvBatchChompRelu(n_outputs,
                                      self.n_outputs_branch,
                                      k,
                                      stride,
                                      dilation,
        ...
        self.downsample = nn.Conv1d(
            n_inputs, n_outputs,
            1) if (n_inputs // self.num_kernels) != n_outputs else None
...
class MultibranchTemporalConvNet(nn.Module):
    def __init__(self,
        ...
        for i in range(num_levels):
            ...
            layers.append(
                MultibranchTemporalBlock(in_channels,
                                         out_channels,
                                         self.ksizes,
                                         stride=1,
    ...
    def __init__(self,
        ...
        if dwpw:
            self.net = nn.Sequential(
                ...
                nn.BatchNorm1d(n_inputs),
                Chomp1d(padding, True),
                nn.PReLU(num_parameters=n_inputs) if relu_type == 'prelu' else
                Swish() if relu_type == 'swish' else nn.ReLU(inplace=True),
                # -- pw
                nn.Conv1d(n_inputs, n_outputs, 1, 1, 0, bias=False),
                nn.BatchNorm1d(n_outputs),
                nn.PReLU(num_parameters=n_outputs) if relu_type == 'prelu' else
                Swish() if relu_type == 'swish' else nn.ReLU(inplace=True),
                nn.Dropout(dropout),
            ...
            self.chomp1 = Chomp1d(padding,
                                  symm_chomp) if not self.no_padding else None
            if relu_type == 'relu':
                self.relu1 = nn.ReLU()
            elif relu_type == 'prelu':
            ...
            if self.no_padding:
                self.net = nn.Sequential(self.conv1, self.batchnorm1,
                                         self.relu1, self.dropout1, self.conv2,
                                         self.batchnorm2, self.relu2,
                                         self.dropout2)
            else:
                self.net = nn.Sequential(self.conv1, self.batchnorm1,
                                         self.chomp1, self.relu1,
                                         self.dropout1, self.conv2,
                                         self.batchnorm2, self.chomp2,
