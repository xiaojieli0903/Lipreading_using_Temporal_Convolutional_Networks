...
__all__ = [
    'Compose', 'Normalize', 'CenterCrop', 'RgbToGray', 'RandomCrop',
...
class RgbToGray(object):
    ...
    def __call__(self, frames):
        ...
        """
        frames = np.stack(
            [cv2.cvtColor(_, cv2.COLOR_RGB2GRAY) for _ in frames], axis=0)
        return frames
...
class AddNoise(object):
    ...
    def __call__(self, signal):
        ...
        if snr_target == 9999:
            ...
            desired_signal = (signal + noise_clip * np.sqrt(factor)).astype(
                np.float32)
