...
class AVSRDataLoader(object):
    ...
    def preprocess(self, video_pathname, landmarks_pathname):
        ...
        if not preprocessed_landmarks or len(
                preprocessed_landmarks) < self._window_margin:
            return
    ...
    def crop_patch(self, video_pathname, landmarks):
        ...
        while True:
            ...
            window_margin = min(self._window_margin // 2, frame_idx,
                                len(landmarks) - 1 - frame_idx)
            smoothed_landmarks = np.mean([
                landmarks[x]
                for x in range(frame_idx - window_margin, frame_idx +
                               window_margin + 1)
            ],
                                         axis=0)
            ...
            frame_idx += 1
        return np.array(sequence), np.array(sequence_frame), np.array(
            sequence_landmarks)
    ...
    def affine_transform(self,
        ...
        transform = cv2.estimateAffinePartial2D(np.vstack(
            [landmarks[x] for x in stable_points]),
                                                stable_reference,
                                                method=cv2.LMEDS)[0]
        ...
        transformed_landmarks = np.matmul(
            landmarks,
            transform[:, :2].transpose()) + transform[:, 2].transpose()
    ...
    def load_data(self, modality, data_filename, landmarks_filename=None):
        ...
        if modality == "raw_audio":
            ...
            return self.load_video(data_filename,
