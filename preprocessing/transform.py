...
def linear_interpolate(landmarks, start_idx, stop_idx):
    ...
    for idx in range(1, stop_idx - start_idx):
        landmarks[
            start_idx +
            idx] = start_landmarks + idx / float(stop_idx - start_idx) * delta
    return landmarks
...
def warp_img(src, dst, img, std_size):
    ...
    """
    tform = tf.estimate_transform('similarity', src,
                                  dst)  # find the transformation matrix
...
def cut_patch(img, landmarks, height, width, threshold=5):
    ...
    cutted_img = np.copy(
        img[int(round(center_y) -
                round(height)):int(round(center_y) + round(height)),
            int(round(center_x) -
                round(width)):int(round(center_x) + round(width))])
    return cutted_img
...
def convert_bgr2gray(sequence):
    ...
    """
    return np.stack([cv2.cvtColor(_, cv2.COLOR_BGR2GRAY) for _ in sequence],
                    axis=0)
